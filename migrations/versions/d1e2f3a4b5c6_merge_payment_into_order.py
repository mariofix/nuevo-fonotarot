...
def upgrade():
    ...
    with op.batch_alter_table("orders", schema=None) as batch_op:
        batch_op.add_column(sa.Column("merchants_id", sa.String(length=128), nullable=True))
        ...
        batch_op.add_column(sa.Column("amount", sa.Numeric(precision=19, scale=4), nullable=True))
        ...
        batch_op.add_column(sa.Column("extra_args", sa.JSON(), server_default=sa.text("'{}'"), nullable=True))
        batch_op.add_column(sa.Column("request_payload", sa.JSON(), server_default=sa.text("'{}'"), nullable=True))
        batch_op.add_column(sa.Column("response_payload", sa.JSON(), server_default=sa.text("'{}'"), nullable=True))
        batch_op.add_column(sa.Column("payment_object", sa.JSON(), server_default=sa.text("'{}'"), nullable=True))
    ...
    # ------------------------------------------------------------------
    with op.batch_alter_table("orders", schema=None) as batch_op:
        batch_op.create_index(batch_op.f("ix_orders_merchants_id"), ["merchants_id"], unique=True)
        batch_op.create_index(batch_op.f("ix_orders_transaction_id"), ["transaction_id"], unique=True)
        batch_op.create_index(batch_op.f("ix_orders_provider"), ["provider"], unique=False)
